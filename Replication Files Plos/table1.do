...
capture erase ${table1}table1_PanelA.xls
capture erase ${table1}table1_PanelA.txt		



*keep  overalllifesat_female recode_diff_wage* worksat_female m11125_female std_mcs_female std_pcs_female  overalllifesat worksat m11125 std_mcs std_pcs  pid  overalllifesat_res worksat_res m11125_res std_mcs_res std_pcs_res dummy running_int 
...
foreach outcomeres in $outcomes_res {
	...
	xi: rdrobust `outcomeres' $running, c($threshold)  vce(cluster $running_or)
	...
	outreg2 using ${table1}table1_PanelA.xls, excel se bdec(3) sdec(3) rdec(3) nocons addtext(state fe) addstat(Mean of Dep. Var., `ymean', Std.Err. of Dep. Var., `ysd') ct(`outcomeres') ti("RDD ROBUST CONTROLLI + FE") append
...
rdrobust overalllifesat_female recode_diff_wage_OR  if age>17&age<60 & (partner==1|partner==2) & female==0, c(0.00001) vce(cluster pid)
rdrobust overalllifesat_female recode_diff_wage_OR  if age>17&age<60 & (partner==1|partner==2) & female==0, c(0.00001) 
rdrobust overalllifesat_female recode_diff_wage_OR  if age>17&age<60 & (partner==1|partner==2) & female==0, c(0.00001) vce(cluster diff_wage_OR)
...
capture erase ${table1}table1_PanelB.xls
capture erase ${table1}table1_PanelB.txt		
...
foreach outcomeres in $outcomes_res {
	...
	outreg2 using ${table1}table1_PanelB.xls, excel se bdec(3) sdec(3) rdec(3) nocons addtext(state fe) addstat(Mean of Dep. Var., `ymean', Std.Err. of Dep. Var., `ysd') ct(`outcomeres') ti("RDD ROBUST") append
