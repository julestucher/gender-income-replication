...
analysis_data %>%
  ...
  dplyr::summarize(
    across(
      all_of(c("age", "wage", "nchildren", "college")),
...
# treatment vector
t = as.vector(analysis_data %>% dplyr::select(treatment))$treatment

# covariate matrix
X = as.matrix(analysis_data %>% dplyr::select(all_of(c("age", "wage", "nchildren", "college"))))

# weights vector from data sources
weights = as.vector(analysis_data %>% dplyr::select(phrf))
